...
@click.command()
@click.option("--nodes", type=click.Path(readable=True), required=False, help="Path to the nodes TSV file.")
...
def main(nodes, report, output_format, verbose, quiet):
    """
    CLI for matrix-validator.
    ...
    try:
        rusty_mv.validate_nodes(nodes, output_format, report)
