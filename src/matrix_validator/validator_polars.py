...
def validate_kg_nodes(nodes, limit):
    ...
    schema_df = (
        pl.scan_csv(nodes, separator="\t", truncate_ragged_lines=True, has_header=True, ignore_errors=True, low_memory=True)
        .limit(10)
        .collect()
    )

    try:
        NodeSchema.validate(schema_df)
    ...
    if not validation_reports:
        ...
        main_df = pl.scan_csv(
            nodes, separator="\t", truncate_ragged_lines=True, has_header=True, ignore_errors=True, low_memory=True
...
def validate_kg_edges(edges, limit):
    ...
    # do an initial schema check
    schema_df = (
        pl.scan_csv(edges, separator="\t", truncate_ragged_lines=True, has_header=True, ignore_errors=True, low_memory=True)
        .limit(10)
        .collect()
    )

    try:
        EdgeSchema.validate(schema_df)
    except pt.exceptions.DataFrameValidationError as ex:
        validation_reports.append("\n".join(f"{e['msg']}: {_display_error_loc(e)}" for e in ex.errors()))
    ...
    if not validation_reports:
        ...
        main_df = pl.scan_csv(
            edges, separator="\t", truncate_ragged_lines=True, has_header=True, ignore_errors=True, low_memory=True
        ).select(usable_columns)
