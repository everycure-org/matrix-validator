...
def validate(df, column, bm_prefixes: list):
    ...
    with_prefix = violations_df.with_columns(
        pl.col(f"invalid_contains_biolink_model_prefix_{column}")
        .str.split(":")
        .list.first()
        .alias("prefix")
    )
    ...
    prefix_counts = with_prefix.group_by("prefix").agg(
    ...
    result = {
        f"invalid_contains_biolink_model_prefix_{column}_summary": report
    }
